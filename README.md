...
[Rattic](https://github.com/tildaslash/RatticWeb/wiki).
...
```
- name: Installation using SQLite, uWSGI and Nginx
  hosts: all
  vars:
    # Make the DB directory and file writable by the uWSGI user
    rattic_db_sqlite_user: 188
    rattic_db_sqlite_group: 187
    # Install Python plugin for uWSGI
    uwsgi_plugins:
      - python
    # Configure uWSGI to run Rattic
    ...
    nginx_vhost_config__default:
      default:
        - server:
            ...
            - location /static:
                - alias {{ rattic_home }}/static
            - location /media:
                - alias {{ rattic_home }}/media
  roles:
    - rattic
    - uwsgi
    - nginx

- name: Installation using PostgreSQL, uWSGI and Nginx
  hosts: all
  vars:
    # Use PostgreSQL engine
    rattic_db_engine: postgresql
    # Rattic configure to use the PostgreSQL DB
...
- [`postgresql`](https://github.com/jtyr/ansible-postgresql) (optinal)
- [`nginx`](https://github.com/jtyr/ansible-nginx) (optinal)
- [`uwsgi`](https://github.com/jtyr/ansible-uwsgi) (optinal)
